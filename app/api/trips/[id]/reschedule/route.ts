...
import { createRescheduleProposal, getReschedulingState } from "@/lib/rescheduling";

export async function GET(_request: Request, { params }: { params: { id: string } }) {
  const access = await authorizeApiUser(); if (!access.ok) return access.response;
  const result = await getReschedulingState(db, access.user.id, params.id);
  return result.ok ? NextResponse.json({ rescheduling: result.value }, { headers: { "Cache-Control": "no-store" } }) : NextResponse.json({ error: result.error }, { status: result.status });
}
