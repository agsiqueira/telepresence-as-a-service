...
import { getJourneyReviewContext, getJourneyReviewState, submitJourneyReview } from "@/lib/journey-reviews";
...
export async function GET(request:Request,{params}:{params:{id:string}}){const access=await authorizeApiUser();if(!access.ok)return access.response;const cursor=new URL(request.url).searchParams.get("cursor")??undefined;return response(await getJourneyReviewContext(db,access.user.id,params.id,cursor))}
