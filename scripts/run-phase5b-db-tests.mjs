...
for (const module of ["marketplace", "profiles", "admin", "phase3-services", "trip-lifecycle"]) { const path = `.phase3-test-build/lib/${module}.js`; writeFileSync(path, readFileSync(path, "utf8").replace('require("server-only");', "")); }
const alias = ".phase3-test-build/node_modules/@/lib"; mkdirSync(alias, { recursive: true }); for (const module of ["marketplace", "profiles", "phase3-services", "trip-lifecycle", "db"]) cpSync(`.phase3-test-build/lib/${module}.js`, `${alias}/${module}.js`);
