...
const read=path=>readFileSync(path,"utf8"),schema=read("prisma/schema.prisma"),migration=read("prisma/migrations/20260731230000_phase4_atomic_agreements/migration.sql"),service=read("lib/agreements.ts"),immediate=read("lib/phase3-services.ts");
...
for(const pattern of [/TransactionIsolationLevel\.Serializable/,/FOR UPDATE/,/status: ProposalStatus\.ACCEPTED/,/status: ProposalStatus\.NOT_SELECTED/,/status: JourneyRequestStatus\.CONVERTED/,/tripId: trip\.id/,/status: TripStatus\.ACCEPTED/,/agreement\.create/,/created: false/])assert.match(service,pattern);assert.match(immediate,/activeTripId: tripId/);
