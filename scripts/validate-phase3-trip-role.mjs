...
for (const file of ["phase3-services.js", "marketplace.js", "trip-lifecycle.js", "safety-restriction-lock.js"]) {
...
writeFileSync(`${root}/safety-restriction-lock.js`, "exports.acquireSafetyRestrictionParticipantLocks = async () => undefined; exports.hasEffectiveSafetyRestrictionInTransaction = async () => false;\n");
...
assert.equal(result.ok, false);
assert.equal(result.status, 409);
assert.equal(mock.state().attempts, 1);
assert.equal(mock.state().userReads, 1);
assert.equal(mock.state().trips.length, 0, "the failed locked transaction does not commit its insert");
