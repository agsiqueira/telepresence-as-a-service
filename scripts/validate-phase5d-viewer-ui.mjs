...
refreshes = 0; controller = createOperatorApplicationController(async () => json({ code: "TELEPORTER_APPLICATION_APPROVED", error: "private" }, 409)); outcome = await controller.submit(valid, async () => { refreshes++; }); assert.equal(outcome.kind, "error"); assert.equal(refreshes, 1); assert.match(outcome.message, /already approved/); assert.doesNotMatch(outcome.message, /private/);
...
const component = readFileSync("components/OperatorApplicationViewer.tsx", "utf8"); const page = readFileSync("app/viewer/operator-application/page.tsx", "utf8"); const layout = readFileSync("app/viewer/layout.tsx", "utf8"); const account = readFileSync("app/viewer/account/page.tsx", "utf8");
assert.match(layout, /requireExplorerPage\(\)/); assert.match(account, /href="\/viewer\/operator-application"/); assert.match(page, /OperatorApplicationViewer/);
assert.match(component, /!pending && !approved/); assert.match(component, /Another application is not required/); assert.match(component, /href="\/operator"/); assert.match(component, /role="alert"/);
assert.match(component, /loadError \? <button/); assert.match(component, /Try loading application status again/);
