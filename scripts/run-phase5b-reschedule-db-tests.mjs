...
for (const module of ["agreements", "rescheduling", "trip-lifecycle", "journey-requests", "marketplace", "marketplace-vocabulary", "profiles", "safety-restriction-lock"]) { const path = `.phase3-test-build/lib/${module}.js`; writeFileSync(path, readFileSync(path, "utf8").replace('require("server-only");', "")); }
const aliasRoot = ".phase3-test-build/node_modules/@/lib"; mkdirSync(aliasRoot, { recursive: true }); for (const module of ["agreements", "rescheduling", "trip-lifecycle", "journey-requests", "marketplace", "marketplace-vocabulary", "profiles", "safety-restriction-lock"]) cpSync(`.phase3-test-build/lib/${module}.js`, `${aliasRoot}/${module}.js`);
