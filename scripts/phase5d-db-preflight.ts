...
if (!process.env.PHASE5D_TEST_DATABASE_URL || process.env.DATABASE_URL !== process.env.PHASE5D_TEST_DATABASE_URL) {
  throw new Error("Unsafe Phase 5D test database mapping");
}
const expected = process.env.PHASE5D_EXPECTED_DATABASE_FINGERPRINT;
if (!expected || expected.length < 16 || expected.length > 128) {
  throw new Error("A valid Phase 5D database fingerprint is required");
}
...
async function main() {
  ...
  try {
    ...
    console.log("Phase 5D disposable database fingerprint verified.");
  } finally {
    await db.$disconnect();
  }
}
