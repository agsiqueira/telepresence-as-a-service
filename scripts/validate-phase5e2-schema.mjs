...
assert.doesNotMatch(migration, /DROP TABLE|DROP COLUMN|DELETE FROM|TRUNCATE|CREATE TRIGGER/); assert.ok(readdirSync("prisma/migrations", { withFileTypes: true }).filter(value => value.isDirectory()).map(value => value.name).includes(migrationName));
