...
function NavigationLinks({ items, onNavigate, mobileTasks = false }: { items: NavigationItem[]; onNavigate?: () => void; mobileTasks?: boolean }) {
  ...
  const mobileGrid = items.length === 5 ? "grid grid-cols-5 gap-1" : "grid grid-cols-4 gap-1";
  return <ul className={mobileTasks ? mobileGrid : "flex flex-col gap-1 md:flex-row md:flex-wrap"}>{items.map(item => { const active = current(pathname, item); return <li key={item.href} className={mobileTasks ? "min-w-0" : undefined}><Link href={item.href} onClick={onNavigate} aria-current={active ? "page" : undefined} className={`inline-flex min-h-control w-full min-w-0 items-center justify-center rounded-md px-1 py-2 text-center text-caption transition-colors duration-fast sm:px-2 md:w-auto md:px-3 md:text-label ${active ? "bg-brand-subtle text-brand underline decoration-2 underline-offset-4" : "text-ink-secondary hover:bg-surface-subtle hover:text-ink"}`}>{item.label}{active && <span className="sr-only">, current page</span>}</Link></li>; })}</ul>;
}
